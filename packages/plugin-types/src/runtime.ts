...
export function normalizeBbox(bbox: unknown): NormalizedBoundingBox | null {
  ...
  if (typeof bbox === 'object') {
    ...
    if ('xmin' in b && 'ymin' in b && 'xmax' in b && 'ymax' in b) {
      const { xmin, ymin, xmax, ymax } = b as { xmin: number; ymin: number; xmax: number; ymax: number };
...
export interface PluginContext {
  ...
  getPreviousNodeResult: (nodeType: string) => Promise<WorkflowNodeResult | null>;
}
...
export interface PluginLifecycleHooks {
  ...
  onWorkflowComplete?(context: PluginContext, results: WorkflowNodeResult[]): Promise<void>;
  onNodeStart?(context: PluginContext, nodeType: string): Promise<void>;
  onNodeComplete?(context: PluginContext, result: WorkflowNodeResult): Promise<void>;
  onNodeError?(context: PluginContext, nodeType: string, error: Error): Promise<void>;
}
